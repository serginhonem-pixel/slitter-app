...
const getIcon = (eventType) => {
    switch (eventType) {
        case 'ENTRADA_MP': return Package;
        case 'CORTE_SLITTER': return Scissors;
        case 'PRODUCAO_PA': return Factory;
        case 'EXPEDICAO_PA': return Truck;
    ...
    return (
        <Card className="lg:col-span-4 flex flex-col">
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                <History size={24} className="text-gray-400" /> Últimos Movimentos
            </h2>
            ...
            <div className="space-y-4 overflow-y-auto custom-scrollbar-dark max-h-[400px]">
                {logs.length === 0 ? (
                    ...
                    logs.map((log, index) => {
                        ...
                        let description = '';
                        if (log.eventType === 'ENTRADA_MP') {
                            description = `Bobina ${log.motherCode} (${log.weight}kg) entrada via NF ${log.nf || 'N/A'}`;
                        } else if (log.eventType === 'CORTE_SLITTER') {
                            description = `Corte da Bobina ${log.motherCode}. Gerou ${log.newChildCount} B2s. Sucata: ${log.scrapKg}kg.`;
                        } else if (log.eventType === 'PRODUCAO_PA') {
                            description = `Produção de ${log.pieces} pçs do produto ${log.productCode}.`;
                        } else if (log.eventType === 'EXPEDICAO_PA') {
                            description = `Expedição de ${log.quantity} pçs do produto ${log.productCode}.`;
                        } else {
                            description = JSON.stringify(log.details);
                        }

                        return (
                            <div key={index} className="flex items-start gap-4 p-3 bg-gray-800/50 rounded-xl border border-gray-700/50 hover:bg-gray-700/50 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-1">{log.date} - {new Date(log.timestamp).toLocaleTimeString('pt-BR')}</p>
                                </div>
                            </div>
                        );
                    })

...
const today = new Date();
  ...
  // Modo MANUAL
const [mpManualInitialStock, setMpManualInitialStock] = React.useState('');
const [mpManualStartDate, setMpManualStartDate] = React.useState(() => {
  const d = new Date();               // ❗ não usa today aqui
  return d.toISOString().slice(0, 10);
});
const [mpManualEndDate, setMpManualEndDate] = React.useState(() => {
  const d = new Date();               // ❗ também não usa today aqui
...
minStock = Number(mpMinStock) || 0;
  ...
  const totalIncoming = activeOrders.reduce((acc,o) => acc + o.qty, 0);
    ...
    // Demanda diária média (usando o valor total / dias no período)
    scenarioDaily = Number(mpManualDemandValue) || 0;
    scenarioDaily = scenarioDaily / daysInPeriod;

    // Estoque inicial: se o campo estiver vazio, usa o estoque atual
...
const inflows = activeOrders.filter((o) => {
  ...
  // força meia-noite pra não dar diferença de fuso
  const oDate = new Date(o.date + 'T00:00:00');
  return sameDay(oDate, simDate);
});

const inflowQty = inflows.reduce((acc, o) => acc + (Number(o.qty) || 0), 0);


      // Saídas (demanda) só dentro do período da simulação manual
      let dailyDemand = 0;
...
if (firstStockoutDate) {
  ...
  // quantos dias faltam para o limite de compra (pode ser negativo)
  daysToDeadline = Math.round(
    (deadline.getTime() - baseDate.getTime()) / MS_PER_DAY
  );

  if (daysToDeadline < 0) {
    // já passou da data limite
    ...
    const getX = (i) =>
      chartPaddingLeft +
      (i / (dailyStatement.length - 1 || 1)) * innerW;
    ...
    // 3. Pedidos em Trânsito (Entradas Futuras)
...
const handleAddOrder = () => {
  ...
  setMpIncomingOrders((prev) => [
    ...prev,
    {
      ...
      date: mpOrderDate,        // formato yyyy-mm-dd vindo do <input type="date">
      qty: Number(mpOrderQty),  // garante número
    },
  ]);
  ...
  return (
      ...
      <div className="flex gap-4 bg-gray-800 p-4 rounded-xl border border-gray-700 items-end">
        <div className="flex-1">
          <h2 className="text-xl font-bold text-white flex gap-2">
            ...
            SIMULADOR DE PLANEJAMENTO DE COMPRAS
          </h2>
        </div>
        ...
        >
          ...
          {[...new Set(groupList.map((g) => g.thickness))]
            .sort()
            .map((t) => (
        ...
        >
          ...
          {[...new Set(groupList.map((g) => g.type))]
            .sort()
            .map((t) => (
              <option key={t}>{t}</option>
            ))}
            ...
            <div className="mt-6 pt-4 border-t border-gray-700 grid grid-cols-4 gap-4">
              ...
              <label className="flex flex-col">
                ...
                <input
                  ...
                  value={mpTargetDays}
                  onChange={(e) =>
                    setMpTargetDays(e.target.value)
                  }
                  className="bg-gray-900 border border-gray-600 rounded px-3 py-2 text-white"
                />
              </label>
    ...
    {selectedGroup && (
      <p className="text-xs text-gray-400 mt-1">
        Mãe: {formatKg(selectedGroup.motherStockWeight || 0)} kg · B2:{' '}
        {formatKg(selectedGroup.b2StockWeight || 0)} kg
      </p>
    )}
  </div>
            <div className="bg-gray-800 p-4 rounded-xl border border-gray-700">
            ...
            <div className="bg-gray-800 p-4 rounded-xl border border-gray-700">
              ...
              {/* AVISO DE LEAD TIME / LIMITE DE COMPRA */}
{ruptureDateStr !== "Sem risco" && (
  ...
  >
    {isLeadTimeCritical ? (
      <>
        Você <strong>já passou</strong> do limite de compra considerando um
        lead time de <strong>{mpLeadTime}</strong> dias. A ruptura está
        prevista para <strong>{ruptureDateStr}</strong>. Ajuste o plano de
        compras ou negocie prazo/estoque com o fornecedor.
      </>
    ) : (
      <>
        Para evitar ruptura, faça o próximo pedido até{" "}
        <strong>{deadlineDateStr}</strong> (lead time{" "}
        <strong>{mpLeadTime}</strong> dias). A ruptura está prevista para{" "}
        <strong>{ruptureDateStr}</strong>.
      </>
    )}
  </div>
)}
                ...
                <circle
                  ...
                  onMouseEnter={() =>
                    console.log(
                      `Data: ${formatDate(
                        p.date
                      )}, Saldo: ${formatKg(p.balance)} kg`
                    )
                  }
                />
              ))}

              {/* Linhas verticais mostrando os pedidos em trânsito */}
{selectedGroup && activeOrders.map((o) => {
  if (!o.date) return null;
  const oDate = new Date(o.date + 'T00:00:00');

  const dayIndex = dailyStatement.findIndex((d) =>
  ...
  const point = graphPointsData[dayIndex];
  if (!point) return null;

  return (
    <g key={o.id}>
      {/* Linha vertical do pedido */}
      <line
        x1={point.x}
        y1={0}
        x2={point.x}
        ...
        strokeWidth="1"
      />
      {/* Label pequena com o tamanho do pedido */}
      <text
        x={point.x + 3}
        y={12}
        fill="#6ee7b7"
